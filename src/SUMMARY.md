# Summary

[Introduction](./intro.md)

- [Cluster Hierarchy](./cluster.md)
- [Service Discovery](./service_discovery.md)
- [Supported Execution Models](./supported_exec_model.md)
- [Request routing](./request_routing.md)
  - [General Design](./request_routing_general.md)
  - [TPC compatible design](./request_routing_tpc.md)
- [Partition management](./partition_management.md)
- [Persistence mechanism](./persistence_mechanism.md)
  - [`segmented_log`](./segmented_log.md)
  - [`indexed_segmented_log`](./indexed_segmented_log.md)
- [Execution Model](./execution_model.md)
  - [General async runtime](./general_async.md)
  - [Thread Per Core](./thread_per_core.md)
- [Testing](./testing.md)
